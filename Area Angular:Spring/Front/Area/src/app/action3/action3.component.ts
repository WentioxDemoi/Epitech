...
export class Action3Component implements OnInit {
  ...
  ngOnInit() {
      ...
      const tmpReaction1 = localStorage.getItem('reaction11')
      if (tmpReaction1)
          this.reaction1Value = JSON.parse(tmpReaction1);
      const tmpReaction2 = localStorage.getItem('reaction12')
      if (tmpReaction2)
          this.reaction2Value = JSON.parse(tmpReaction2);
      const tmpReaction3 = localStorage.getItem('reaction13')
      if (tmpReaction3)
          this.reaction3Value = JSON.parse(tmpReaction3);
      const tmpReaction4 = localStorage.getItem('reaction14')
      if (tmpReaction4)
          this.reaction4Value = JSON.parse(tmpReaction4);
          const tmpReaction5 = localStorage.getItem('reaction15')
      if (tmpReaction5)
          this.reaction5Value = JSON.parse(tmpReaction5);
      const tmpButtonDisabled = localStorage.getItem('buttonDisabled3')
      if (tmpButtonDisabled) {
        this.buttonDisabled3 = JSON.parse(tmpButtonDisabled);
      } else {
  ...
  requete() {
    ...
    this.ObjectService.addObject(this.object).subscribe((response: any) => {
        if (response) {
            localStorage.setItem('reaction11', JSON.stringify(this.reaction1Value));
            localStorage.setItem('reaction12', JSON.stringify(this.reaction2Value));
            localStorage.setItem('reaction13', JSON.stringify(this.reaction3Value));
            localStorage.setItem('reaction14', JSON.stringify(this.reaction4Value));
            localStorage.setItem('reaction15', JSON.stringify(this.reaction4Value));
            ...
            localStorage.setItem('buttonDisabled3', JSON.stringify(this.buttonDisabled3));
        }
    })
    this.reactionTotal = [];
    }
